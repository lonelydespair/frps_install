...
DOWNLOAD() (
	echo -e "\n${GREEN_COLOR}Download "$mirror"https://github.com/sbwml/luci-app-alist/releases/v3.41.0/download/openwrt-$sdk-$platform.tar.gz ...${RES}\n"
	curl --connect-timeout 5 -m 300 -kLo "$temp_dir/openwrt-$sdk-$platform.tar.gz" "$mirror"https://github.com/sbwml/luci-app-alist/releases/v3.41.0/download/openwrt-$sdk-$platform.tar.gz
	if [ $? -ne 0 ]; then
